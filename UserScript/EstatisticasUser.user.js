...
const TWStatsBaseURL="https://pt.twstats.com/";
...
function generateTBody(data, key) {
    var world = new URL(document.URL).hostname.match(/^([^.]+)\./)?.[1];
    ...
    image_pontos.src = TWStatsBaseURL+"image.php?type=playergraph&graph=points&id=" + key + "&s=" + world;
    ...
    image_oda.src =  TWStatsBaseURL+"image.php?type=playergraph&graph=oda&id=" + key + "&s=" + world;

    td_oda.appendChild(image_oda);

    var td_odd = document.createElement("td");
    var image_odd = document.createElement("img");
    image_odd.src =  TWStatsBaseURL+"image.php?type=playergraph&graph=odd&id=" + key + "&s=" + world;
